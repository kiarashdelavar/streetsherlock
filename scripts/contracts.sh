...
run_pnpm() {
  if command -v pnpm >/dev/null 2>&1; then
    ...
    printf 'pnpm is required. Install it or run through Corepack.\n' >&2
    return 127
  fi
}
...
generate() {
  ...
  run_pnpm --silent dlx "$GENERATOR" "$SPEC" -o "$GENERATED"
}
...
check() {
  ...
  run_pnpm --silent dlx "$GENERATOR" "$SPEC" -o "$temp_dir/api-types.ts"

  if ! cmp -s "$temp_dir/api-types.ts" "$GENERATED"; then
    printf '%s\n' \
      'Generated API types are stale.' \
      'Run: pnpm contracts:generate' >&2

...
exact() {
  ...
  if [[ "$actual" != "$expected" ]]; then fail "$label $actual is unsupported; expected $expected."
  else printf 'OK: %s %s\n' "$label" "$actual"; fi
}
at_least() {
...
if need java 'Install a Java 21 JDK; .sdkmanrc provides a Temurin candidate.'; then
  ...
  exact Java-major "${java_version%%.*}" "$JAVA_MAJOR"
fi
if need mvn 'Install Maven 3.9.11; .tool-versions pins the supported version.'; then
  exact Maven "$(mvn --version | awk 'NR==1 {print $3}')" "$MAVEN_VERSION"
fi
...
if need docker 'Install Docker Engine or Docker Desktop before service work.'; then
  ...
  if [[ -z "$docker_version" ]]; then fail 'Docker CLI exists but the client version could not be read.'
  else at_least Docker-client "$docker_version" "$DOCKER_MIN_VERSION"; fi
  compose_version="$(docker compose version --short 2>/dev/null || true)"
  compose_version="${compose_version#v}"
  if [[ -z "$compose_version" ]]; then fail 'Docker Compose v2 is missing. Install the docker compose plugin.'
  else at_least Docker-Compose "$compose_version" "$COMPOSE_MIN_VERSION"; fi
fi
